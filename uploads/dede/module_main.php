...
function ReWriteConfigAuto()
{
    ...
    {
        if($row['type']=='number') fwrite($fp,"\${$row['varname']} = ".intval($row['value']).";\r\n");
        else fwrite($fp,"\${$row['varname']} = '".str_replace("'",'', dede_htmlspecialchars($row['value']))."';\r\n");
    }
    fwrite($fp,"?".">");
    fclose($fp);
}
...
else if($action=='uninstallok')
{
    ...
    else
    {
        ...
        if(trim($delsql)!='')
        {
            ...
            {
                // 恶意代码
                $sql = str_replace('general_log', '恶意代码', $sql);
